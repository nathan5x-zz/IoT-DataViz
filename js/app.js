...
$(document).ready(function(){
  ...
  var previousLevel = 0;
  var currentLevel = 0;
  var soilMoisturePollTimer = window.setInterval( getSoilData, 3000);

  function getSoilData() {
    $.ajax({
      url: "https://data.sparkfun.com/output/jqxOJWGdXmFvZZXjOm6o.json",
      dataType: "jsonp",
      data: {page: 1},
      success: function( response ) {
  ...
  function renderSoilVisualization( moistureData) {
    ...
    var wLevel = Math.random() * (250 - 0) + 0; //Math.floor( moistureData.photosensor * .08 ); //
  ...
  $soilColChart.series[0].addPoint({
      y: parseFloat(wLevel),
      name: "10/12/2015 3:14 AM"}, true, true);
      //name: moistureData.timestamp}, true, true);
  }
